...
"""
############################################################################################################ SOFTWARE STRUCTURE
# PyQt worker threads coordinate acquisition, scan motion, processing, display, and saving.
# Queues carry lightweight action objects and shared-memory slot indices between threads.
# Raw spectral data stay in shared memory; downstream workers reference memory slots instead of copying raw arrays.
############################################################################################################ HARDWARE STRUCTURE
# The swept-source A-line trigger drives camera timing and scan-board timing.
# X motion is generated by the galvo waveform and Y motion is generated by the stage.
# Stage motion uses buffered STEP pulses plus a non-buffered DIRECTION signal.
