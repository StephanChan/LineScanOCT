...
class Camera(QThread):
    ...
    def ConfigureBoard(self):
        ...
        elif self.ui.ACQMode.currentText() in ['SingleCscan']:
            self.BlinesPerAcq = self.ui.Ypixels.value() 
        ...
        if self.ui.ACQMode.currentText() in ['SingleBline', 'SingleAline','SingleCscan']:
    ...
    def StartAcquire(self):
        ...
        while BlinesCount < self.BlinesPerAcq and self.ui.RunButton.isChecked():
            ...
            # handle pause action
            if self.ui.PauseButton.isChecked() and self.ui.ACQMode.currentText() in ['RptBline', 'RptAline']:
    ...
    def simData(self):
        ...
        while BlinesCount < self.BlinesPerAcq and self.ui.RunButton.isChecked():
            ...
            if self.ui.PixelFormat_display.text() in ['Mono8']:
                Bline = np.uint8(np.random.rand(self.ui.Height.value(), self.AlinesPerBline)*255)
            else:
                
                Bline = np.uint16(np.random.rand(self.ui.Height.value(), self.AlinesPerBline)*65535)
            # print(Bline[0,0:5])
